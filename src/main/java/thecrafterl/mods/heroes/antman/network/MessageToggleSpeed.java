...
public class MessageToggleSpeed implements IMessage, IMessageHandler<MessageToggleSpeed, IMessage> {
    
    public MessageToggleSpeed() {
    ...
    @Override
    public IMessage onMessage(MessageToggleSpeed msg, MessageContext ctx) {
        EntityPlayer player = getPlayerEntity(ctx);
        
        if (player != null) {
    		if(AntMan.isSmall(player)) {
    			AntMan.toggleSpeed(player);
    		}
        }
        return null;
    }
